...
- Package: obsidian_merger (src layout)
- Tooling: uv-managed environment, pytest, ruff, mypy, hatchling build backend
- Entry points (after install):
  - obsidian-merge -> obsidian_merger.cli:main (Typer CLI)
  - obsidian-merge-gui -> obsidian_merger.gui:main (PySimpleGUI GUI)
...
- Run the CLI without installing the package:
  - uv run python -m obsidian_merger.cli --help
  - uv run python -m obsidian_merger.cli plan -s /path/to/vaultA -s /path/to/vaultB -t /path/to/merged -o merge-plan.json
  - uv run python -m obsidian_merger.cli apply merge-plan.json
  - uv run python -m obsidian_merger.cli gui -s /path/to/vaultA -s /path/to/vaultB -t /path/to/merged
- Launch the GUI (after install, console script available):
  - uv run obsidian-merge-gui
  Notes: GUI uses PySimpleGUI and requires a desktop session. Prefer the CLI for headless use.
- Tests (pytest):
  - Run all tests: uv run pytest
  - Run with coverage: uv run pytest --cov=obsidian_merger --cov-report=term-missing
...
- CLI (Typer) — src/obsidian_merger/cli.py
  - Commands:
...
- Planner — src/obsidian_merger/planner.py
...
- Apply engine — src/obsidian_merger/apply.py
...
- GUI — src/obsidian_merger/gui.py (PySimpleGUI)
...
- Package metadata — src/obsidian_merger/__init__.py
...
- Verify console scripts: uv run obsidian-merge --help
